...
namespace SDT
{
	...
	void DPapyrus::PostLoadConfig()
	{
		...
		if (m_conf.dynbudget_enabled)
		{
			...
			else
			{
				...
				Message("UpdateBudgetBase: %.6g ms (%.6g - %.6g)", m_conf.dynbudget_base, m_t_min * m_bmult, m_t_max * m_bmult);
			}
		}
	}
	...
	const wchar_t* DPapyrus::StatsRendererCallback1()
	{
		...
		{
			::_snwprintf_s(m_Instance.m_bufStats1, _TRUNCATE, L"fUpdateBudgetMS: %.4g", val);
		}
